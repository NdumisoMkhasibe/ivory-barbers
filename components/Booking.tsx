  ...
  return <section id="book" className="booking-section" aria-labelledby="book-title">
    ...
    <div className="booking-shell">
      {confirmation ? <div className="booking-confirmation">
        ...
        <div className="booking-calendar-actions"><a className="booking-button" href={confirmation.googleCalendarUrl} target="_blank" rel="noopener noreferrer">Add to Google Calendar <span aria-hidden="true">↗</span></a><a className="booking-button booking-button-outline" href={confirmation.icsUrl} download>Download for Apple Calendar <span className="booking-ics">.ics ↓</span></a></div>
        <p className="booking-note">This is a fictional portfolio concept. No real appointment or payment is offered. We do not send email or SMS confirmations.</p>
        <button className="booking-text-button" type="button" onClick={resetBooking}>Make another demo booking <span aria-hidden="true">→</span></button>
      </div> : <>
        ...
        <div className="booking-layout"><div className="booking-main">
          ...
          {step === 0 && <>
            ...
            <div className="booking-actions"><span className="booking-helper">Choose one to continue</span><button className="booking-button" type="button" disabled={!barberId} onClick={() => goTo(1)}>Choose your service <span aria-hidden="true">→</span></button></div>
          </>}
          {step === 1 && <>
            ...
            <div className="booking-actions"><button className="booking-text-button" type="button" onClick={() => goTo(0)}>← Back</button><button className="booking-button" type="button" disabled={!serviceId} onClick={() => goTo(2)}>Find a time <span aria-hidden="true">→</span></button></div>
          </>}
          {step === 2 && <>
            <div className="booking-date-layout"><div className="booking-calendar"><div className="booking-month"><button type="button" aria-label="Previous month" disabled={month <= today.slice(0, 7)} onClick={() => shiftMonth(-1)}>←</button><h4 aria-live="polite">{monthTitle}</h4><button type="button" aria-label="Next month" onClick={() => shiftMonth(1)}>→</button></div><div className="booking-weekdays" aria-hidden="true">{["M", "T", "W", "T", "F", "S", "S"].map((label, index) => <span key={index}>{label}</span>)}</div><div className="booking-days" role="group" aria-label="Choose your appointment date">{calendarDays.map(({ day, iso, inMonth }) => <button key={iso} type="button" disabled={!inMonth || iso < today || day.getDay() === 0} className={`${date === iso ? "is-selected" : ""} ${iso === today ? "is-today" : ""} ${!inMonth ? "is-outside" : ""}`} aria-label={`${dateLabel(iso)}${day.getDay() === 0 ? ", closed" : ""}${iso === today ? ", today" : ""}`} aria-pressed={date === iso} onClick={() => chooseDate(iso)}>{day.getDate()}</button>)}</div><p className="booking-calendar-note">Mon–Fri 09:00–18:00 · Sat 08:00–16:00<br />Sundays are for slowing down. We’re closed.</p></div>
            ...
            <div className="booking-actions"><button className="booking-text-button" type="button" onClick={() => goTo(1)}>← Back</button><button className="booking-button" type="button" disabled={!date || !time || loadingSlots || !!slotsError} onClick={() => goTo(3)}>Your details <span aria-hidden="true">→</span></button></div>
          </>}
